...
const AddAutre : React.FC<AddAutreProps> = ({autres, utilisateur, salles, addNewAutre}) => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!IsOverlapping(autre)) {
      // autreService.createAutre(autreCapitalized)
      //   .then(() => onUpdateAutre(autreCapitalized))
      //   .then(() => setAutre({ id: 0, type: 'Entretien', salle: newSalle, desc: '', createdAt: new Date(), startAt: new Date(), endAt: new Date(), utilisateur: utilisateur}))
    ...
    <>
      <form className='addFormSectionAutres' onSubmit={(event) => handleSubmit(event)}>
        ...
        <section className="addFicheLeftSectionAutres">
          <div className="inputDivBoxAutres">
            <div className='inputDivAutres'>
